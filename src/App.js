...
export default function App() {
  ...
  const createNewFile=(folderId)=>{const name=prompt('새 파일 이름:');if(!name)return;const f={id:`file_${Date.now()}`,name,pages:[{id:'p1',elements:[]}],lastModified:new Date().toISOString().split('T')[0]};setFolders(p=>p.map(fo=>fo.id===folderId?{...fo,files:[...fo.files,f]}:fo));};
  if(view==='dashboard') return(
    <div className={`min-h-screen p-10 ${T.bg} ${T.text}`}>
      <header className="flex justify-between items-center mb-12 max-w-6xl mx-auto">
        ...
        <div className={`flex items-center p-1 rounded-full ${T.panel} border ${T.border}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {folders.map(folder=>(
          <div key={folder.id} className={`${T.panel} p-6 rounded-2xl border ${T.border} shadow-sm`}>
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={()=>createNewFile(folder.id)} className="p-1.5 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 rounded-md transition"><FilePlus size={16}/></button>
            </div>
  ...
  const updateFileData=(updatedFile)=>setFolders(prev=>prev.map(f=>f.id===currentFolderId?{...f,files:f.files.map(fi=>fi.id===currentFileId?updatedFile:fi)}:f));
  return <Editor file={currentFile} updateFile={updateFileData} goBack={()=>setView('dashboard')} T={T}/>;
}

// ── EDITOR ──
function Editor({file,updateFile,goBack,T}){
  ...
  const [vmTrigger,setVmTrigger]=useState(0);
  ...
  useEffect(()=>{const s=localStorage.getItem('portfolio_assets');if(s)setGlobalAssets(JSON.parse(s));const t=localStorage.getItem('portfolio_user_templates');if(t)setUserTemplates(JSON.parse(t));},[]);
  ...
  useEffect(()=>{if(viewportRef.current){const vW=viewportRef.current.clientWidth-80,vH=viewportRef.current.clientHeight-120;const s=Math.min(vW/canvasSize.width,vH/canvasSize.height);setZoom(s>1?1:s);}},[canvasSize,vmTrigger]);
  useEffect(()=>{
    const h=(e)=>{
      if((e.key==='Delete'||e.key==='Backspace')&&selectedIds.length>0&&e.target.tagName!=='TEXTAREA'&&e.target.tagName!=='INPUT'){updateElements(elements.filter(el=>!selectedIds.includes(el.id)));setSelectedIds([]);}
  ...
  const addTextElement=()=>addElement({type:'text',content:'텍스트를 입력하세요',width:300,height:80,fontFamily:fontOptions[0].value,fontSize:24,fontWeight:'normal',color:'#374151',backgroundColor:'transparent'});
  ...
  const handleImageUpload=(e)=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=(ev)=>{const src=ev.target.result;setGlobalAssets(prev=>[{id:`asset_${Date.now()}`,src,name:f.name,type:'image'},...prev]);addElement({type:'image',src,width:200,height:200});};r.readAsDataURL(f);};
  ...
  // ── 거리 가이드 (피그마 스타일: 선택요소↔모든요소 4방향) ──
  const renderDistanceGuides=()=>{
    if(!selectedEl)return null;
    const S=selectedEl;
    // 선택 요소의 4개 엣지
    const SL=S.x, SR=S.x+S.width, ST=S.y, SB=S.y+S.height;
    ...
    elements.forEach(el=>{
      ...
      // 수평 정렬 (겹치는 Y 범위 있을 때) → 좌우 거리
      const overlapY=ST<EB&&SB>ET;
      if(overlapY){
        // 오른쪽: 선택요소 오른쪽 → 대상요소 왼쪽
        if(EL>SR){const gap=EL-SR;guides.push(
          <div key={`R-${el.id}`} className="absolute pointer-events-none z-40" style={{left:SR,top:SCY-0.5,width:gap,height:1}}>
            <div className="w-full h-px bg-red-400"/>
            <div className="absolute bg-red-500 text-white text-[9px] font-bold px-1 py-0.5 rounded" style={{left:'50%',top:-10,transform:'translateX(-50%)'}}>{Math.round(gap)}</div>
          </div>
        );}
        // 왼쪽: 대상요소 오른쪽 → 선택요소 왼쪽
        if(ER<SL){const gap=SL-ER;guides.push(
          <div key={`L-${el.id}`} className="absolute pointer-events-none z-40" style={{left:ER,top:SCY-0.5,width:gap,height:1}}>
            <div className="w-full h-px bg-red-400"/>
            <div className="absolute bg-red-500 text-white text-[9px] font-bold px-1 py-0.5 rounded" style={{left:'50%',top:-10,transform:'translateX(-50%)'}}>{Math.round(gap)}</div>
          </div>
        );}
      }
      // 수직 정렬 (겹치는 X 범위 있을 때) → 상하 거리
      const overlapX=SL<ER&&SR>EL;
      if(overlapX){
        // 아래: 선택요소 아래 → 대상요소 위
        if(ET>SB){const gap=ET-SB;guides.push(
          <div key={`D-${el.id}`} className="absolute pointer-events-none z-40" style={{left:SCX-0.5,top:SB,width:1,height:gap}}>
            <div className="h-full w-px bg-red-400"/>
            <div className="absolute bg-red-500 text-white text-[9px] font-bold px-1 py-0.5 rounded" style={{top:'50%',left:4,transform:'translateY(-50%)'}}>{Math.round(gap)}</div>
          </div>
        );}
        // 위: 대상요소 아래 → 선택요소 위
        if(EB<ST){const gap=ST-EB;guides.push(
          <div key={`U-${el.id}`} className="absolute pointer-events-none z-40" style={{left:SCX-0.5,top:EB,width:1,height:gap}}>
            <div className="h-full w-px bg-red-400"/>
            <div className="absolute bg-red-500 text-white text-[9px] font-bold px-1 py-0.5 rounded" style={{top:'50%',left:4,transform:'translateY(-50%)'}}>{Math.round(gap)}</div>
          </div>
        );}
...
요소 6~12개. 좌표는 캔버스 범위 내.`;
  ...
  const exportPDF=async()=>{if(!canvasRef.current)return;setSelectedIds([]);setShowCenterLines(false);setShowGrid(false);setTimeout(async()=>{const canvas=await html2canvas(canvasRef.current,{scale:2});const pdf=new jsPDF(canvasSize.width>canvasSize.height?'l':'p','px',[canvasSize.width,canvasSize.height]);pdf.addImage(canvas.toDataURL('image/png'),'PNG',0,0,canvasSize.width,canvasSize.height);pdf.save(`${file.name}_page${currentPageIndex+1}.pdf`);},100);};
  ...
  const renderToolbar=()=>{
    ...
    const TextTab=()=>{
      ...
      return(
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <select value={ts.fontFamily||fontOptions[0].value} onChange={e=>upd({fontFamily:e.target.value})} className="bg-transparent hover:bg-gray-800 text-xs px-2 py-1 rounded outline-none">
            {fontOptions.map(f=><option key={f.name} value={f.value} className="bg-gray-800">{f.name}</option>)}
          </select>
          <input type="number" value={ts.fontSize||16} onChange={e=>upd({fontSize:Number(e.target.value)})} className="w-10 bg-transparent text-xs text-center outline-none hover:bg-gray-800 rounded py-1 border border-gray-700"/>
          <button onClick={()=>upd({fontWeight:ts.fontWeight==='bold'?'normal':'bold'})} className={`p-1.5 rounded ${ts.fontWeight==='bold'?'bg-gray-700 text-emerald-400':'hover:bg-gray-800'}`}><Bold size={13}/></button>
          <button onClick={()=>upd({fontStyle:ts.fontStyle==='italic'?'normal':'italic'})} className={`p-1.5 rounded ${ts.fontStyle==='italic'?'bg-gray-700 text-emerald-400':'hover:bg-gray-800'}`}><Italic size={13}/></button>
          <button onClick={()=>upd({textDecoration:ts.textDecoration==='underline'?'none':'underline'})} className={`p-1.5 rounded ${ts.textDecoration==='underline'?'bg-gray-700 text-emerald-400':'hover:bg-gray-800'}`}><Underline size={13}/></button>
          <div className="relative w-7 h-7 rounded-full overflow-hidden border border-gray-600 cursor-pointer"><div className="w-full h-full rounded-full" style={{backgroundColor:ts.color||'#374151'}}/><input type="color" value={ts.color||'#374151'} onChange={e=>upd({color:e.target.value})} className="absolute inset-0 opacity-0 cursor-pointer w-full h-full"/></div>
          {!isShape&&<>
            <div className="relative w-7 h-7 rounded overflow-hidden border border-gray-600 cursor-pointer"><div className="w-full h-full" style={{backgroundColor:selectedEl.backgroundColor==='transparent'?'#fff':selectedEl.backgroundColor||'#fff'}}/><input type="color" value={selectedEl.backgroundColor==='transparent'?'#ffffff':selectedEl.backgroundColor||'#ffffff'} onChange={e=>updateElement(selectedId,{backgroundColor:e.target.value})} className="absolute inset-0 opacity-0 cursor-pointer w-full h-full"/></div>
    ...
    const ArrangeTab=()=>(
      <div className="flex items-center gap-1 flex-wrap">
        <button onClick={bringForward} title="앞으로" className="p-1.5 hover:bg-gray-800 text-gray-300 rounded"><ArrowUpToLine size={14}/></button>
        <button onClick={sendBackward} title="뒤로" className="p-1.5 hover:bg-gray-800 text-gray-300 rounded"><ArrowDownToLine size={14}/></button>
        <div className="w-px h-4 bg-gray-700 mx-1"/>
        <button onClick={()=>updateElement(selectedId,{x:(canvasSize.width-selectedEl.width)/2})} title="수평 중앙" className="p-1.5 hover:bg-gray-800 text-gray-300 rounded"><MoveHorizontal size={14}/></button>
        <button onClick={()=>updateElement(selectedId,{y:(canvasSize.height-selectedEl.height)/2})} title="수직 중앙" className="p-1.5 hover:bg-gray-800 text-gray-300 rounded"><MoveVertical size={14}/></button>
        <div className="w-px h-4 bg-gray-700 mx-1"/>
        {selectedIds.length>=2&&<button onClick={groupSelected} title="그룹(Ctrl+G)" className="p-1.5 hover:bg-gray-800 text-emerald-400 rounded"><Group size={14}/></button>}
        {selectedEl?.type==='group'&&<button onClick={ungroupSelected} title="그룹해제(Ctrl+Shift+G)" className="p-1.5 hover:bg-gray-800 text-amber-400 rounded"><Ungroup size={14}/></button>}
      </div>
    );
    ...
    return(
      <div className="flex flex-col items-center gap-1" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center gap-0.5 bg-gray-800/90 rounded-xl px-1 py-0.5 backdrop-blur-md shadow-lg">
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setToolbarTab(t.id)}
              className={`px-3 py-1 text-xs rounded-lg font-medium transition ${activeTab===t.id?'bg-emerald-600 text-white':'text-gray-400 hover:text-gray-200'}`}>
              {t.label}
            </button>
          ))}
          <div className="w-px h-4 bg-gray-700 mx-1"/>
          <button onClick={()=>{updateElements(elements.filter(el=>!selectedIds.includes(el.id)));setSelectedIds([]);}} className="p-1.5 hover:bg-red-500/20 text-red-400 rounded"><Trash2 size={13}/></button>
        </div>
        <div className="bg-gray-900/95 backdrop-blur-md border border-gray-700 text-white px-3 py-2 rounded-xl shadow-2xl min-w-max">
  ...
  const renderElement=(el)=>{
    if(el.type==='text')return(<div className="w-full h-full relative group"><div className="drag-handle absolute -top-5 left-0 w-full h-5 cursor-move opacity-0 group-hover:opacity-100 bg-emerald-500 flex items-center justify-center rounded-t-md"><Maximize size={9} className="text-white"/></div><textarea value={el.content} onChange={e=>updateElement(el.id,{content:e.target.value})} style={{fontFamily:el.fontFamily,fontSize:`${el.fontSize}px`,fontWeight:el.fontWeight,fontStyle:el.fontStyle,textDecoration:el.textDecoration,color:el.color,backgroundColor:el.backgroundColor}} className="w-full h-full resize-none outline-none p-1 bg-transparent"/></div>);
    ...
    if(el.type==='image')return(<div className="w-full h-full drag-handle cursor-move"><img src={el.src} alt="" className="w-full h-full object-cover pointer-events-none"/></div>);
  ...
  return(
    <div className={`flex h-screen w-full transition-colors ${T.bg} ${T.text}`} onClick={()=>setSelectedIds([])}>

      {/* 사이드바 w-72 = 288px */}
      <aside className={`w-72 flex flex-col ${T.panel} border-r ${T.border} z-20 shadow-xl shrink-0`} onClick={e=>e.stopPropagation()}>
        <div className="p-4 border-b border-white/5 shrink-0">
          <button onClick={goBack} className="flex items-center gap-2 text-sm text-gray-500 hover:text-emerald-500 font-medium transition-colors"><ChevronLeft size={18}/>대시보드</button>
        </div>
        <div className={`flex border-b ${T.border} shrink-0`}>
          {[{id:'tools',label:'도구'},{id:'assets',label:'에셋'},{id:'templates',label:'템플릿'}].map(t=>(
            <button key={t.id} onClick={()=>setSidebarTab(t.id)} className={`flex-1 py-2.5 text-xs font-bold transition ${sidebarTab===t.id?'text-emerald-500 border-b-2 border-emerald-500':'text-gray-500 hover:text-gray-300'}`}>{t.label}</button>
          ))}
        </div>
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
          {sidebarTab==='tools'&&<>
            <button onClick={addTextElement} className={`py-2.5 rounded-xl border ${T.border} hover:border-emerald-500/50 flex justify-center items-center gap-2 font-medium text-sm`}><Type size={16}/>텍스트 박스</button>
            <div className={`rounded-xl border ${T.border} overflow-hidden`}>
              <button onClick={()=>setShapesPanelOpen(v=>!v)} className="w-full py-2.5 px-4 flex items-center justify-between font-medium text-sm hover:bg-white/5">
                <span className="flex items-center gap-2"><Square size={15}/>도형</span>
                <ChevronDown size={13} className={`transition-transform ${shapesPanelOpen?'rotate-180':''}`}/>
              </button>
            ...
            <div className={`rounded-xl border ${T.border} overflow-hidden`}>
              <button onClick={()=>setLinePanelOpen(v=>!v)} className="w-full py-2.5 px-4 flex items-center justify-between font-medium text-sm hover:bg-white/5">
                <span className="flex items-center gap-2"><Minus size={15}/>선 / 화살표</span>
                <ChevronDown size={13} className={`transition-transform ${linePanelOpen?'rotate-180':''}`}/>
              </button>
              {linePanelOpen&&<div className="p-3 border-t border-white/5 flex flex-col gap-2">
                ...
                <button onClick={addLineElement} className="py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-medium transition">선 추가</button>
              </div>}
            </div>
            <button onClick={()=>fileInputRef.current.click()} className="py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl flex justify-center items-center gap-2 font-medium text-sm"><Upload size={16}/>PC 사진 등록</button>
          ...
          {sidebarTab==='assets'&&<>
            ...
            <div className="grid grid-cols-2 gap-2">{globalAssets.map(asset=>(<div key={asset.id} onClick={()=>addElement({type:'image',src:asset.src,width:200,height:200})} className="group relative cursor-pointer rounded-xl overflow-hidden border border-transparent hover:border-emerald-500/30 aspect-square bg-black/10"><img src={asset.src} alt={asset.name} className="w-full h-full object-cover"/><div className="absolute inset-0 bg-gray-900/60 opacity-0 group-hover:opacity-100 flex items-center justify-center backdrop-blur-sm transition-opacity"><span className="text-xs font-bold text-white">불러오기</span></div></div>))}</div>
          </>}
          {sidebarTab==='templates'&&<>
            <div className={`p-3 rounded-xl border ${T.border} flex flex-col gap-2`}>
              ...
              <div className="flex gap-1"><input value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} placeholder="직접 입력..." className={`flex-1 text-xs px-2 py-1.5 rounded bg-black/20 border ${T.border} outline-none`}/><button onClick={()=>aiPrompt&&generateAITemplate(aiPrompt)} disabled={aiLoading||!aiPrompt} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs rounded transition disabled:opacity-40">{aiLoading?'...':'생성'}</button></div>
            </div>
            <button onClick={saveAsUserTemplate} className={`py-2.5 rounded-xl border ${T.border} hover:border-emerald-500/50 flex justify-center items-center gap-2 text-sm font-medium`}><Save size={15}/>현재 페이지를 템플릿으로 저장</button>
            {userTemplates.length>0&&<><p className="text-xs text-gray-500">저장된 템플릿</p><div className="flex flex-col gap-1.5">{userTemplates.map(tpl=>(<div key={tpl.id} className={`flex items-center justify-between p-2.5 rounded-xl border ${T.border} hover:border-emerald-500/30 transition`}><div><p className="text-sm font-medium">{tpl.name}</p><p className="text-[10px] text-gray-500">{tpl.elements.length}개 요소</p></div><div className="flex gap-1"><button onClick={()=>loadUserTemplate(tpl)} className="p-1.5 hover:bg-emerald-500/10 text-emerald-400 rounded text-xs">불러오기</button><button onClick={()=>setUserTemplates(prev=>prev.filter(t=>t.id!==tpl.id))} className="p-1.5 hover:bg-red-500/10 text-red-400 rounded"><X size={12}/></button></div></div>))}</div></>}
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        {/* 헤더 h-14 = 56px */}
        <header className={`h-14 flex items-center justify-between px-4 ${T.panel} border-b ${T.border} z-20 shrink-0`} onClick={e=>e.stopPropagation()}>
          <div className="font-semibold text-sm truncate max-w-[180px]">{file.name}</div>
          <div className="flex items-center gap-1.5 bg-black/5 p-1 rounded-lg border border-gray-500/10">
            <div className="flex items-center px-2 border-r border-gray-500/20"><Monitor size={13} className="text-gray-400 mr-1.5"/>
              <select className="bg-transparent text-xs font-medium outline-none cursor-pointer text-emerald-500" value={canvasSizeKey} onChange={e=>{setCanvasSizeKey(e.target.value);setCanvasSize(canvasPresets[e.target.value]);setVmTrigger(p=>p+1);}}>
                {Object.entries(canvasPresets).map(([k,v])=><option key={k} value={k} className="bg-gray-800">{v.label}</option>)}
              </select>
            </div>
            {canvasSizeKey==='CUSTOM'&&<div className="flex items-center gap-1 text-xs text-gray-500 px-2 border-r border-gray-500/20">W:<input type="number" value={canvasSize.width} onChange={e=>{setCanvasSize(c=>({...c,width:Number(e.target.value)}));setVmTrigger(p=>p+1);}} className="w-14 bg-transparent text-center border-b border-gray-500 outline-none"/>H:<input type="number" value={canvasSize.height} onChange={e=>{setCanvasSize(c=>({...c,height:Number(e.target.value)}));setVmTrigger(p=>p+1);}} className="w-14 bg-transparent text-center border-b border-gray-500 outline-none"/></div>}
            <button onClick={()=>setShowGrid(v=>!v)} className={`px-2.5 py-1 rounded text-xs font-medium flex items-center gap-1 ${showGrid?'bg-white/10 text-emerald-400':'text-gray-500'}`}><Grid size={13}/>눈금</button>
            <button onClick={()=>setShowCenterLines(v=>!v)} className={`px-2.5 py-1 rounded text-xs font-medium flex items-center gap-1 ${showCenterLines?'bg-white/10 text-emerald-400':'text-gray-500'}`}><Maximize size={13}/>스냅</button>
          </div>
          <button onClick={exportPDF} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-600 hover:bg-emerald-500 text-white transition-all"><Download size={14}/>PDF 내보내기</button>
        </header>

        {/* 플로팅 툴바: fixed, 헤더(56px) + 사이드바(288px) 반영 */}
        {selectedEl&&(
          <div
            className="fixed z-50 flex flex-col items-center"
            style={{top:'58px', left:'calc(288px + (100vw - 288px)/2)', transform:'translateX(-50%)'}}
        ...
        {/* 뷰포트 */}
        <div ref={viewportRef} className="flex-1 overflow-auto bg-gray-900/5 flex justify-center items-start pt-20 pb-10 px-10 relative" onClick={()=>setSelectedIds([])}>
          <div style={{transform:`scale(${zoom})`,transformOrigin:'top center',transition:'transform 0.2s ease-in-out'}}>
            <div id="portfolio-canvas" ref={canvasRef}
              ...
              {elements.map(el=>(
                ...
                >
                  {selectedIds.includes(el.id)&&<div className="absolute -top-6 left-0 bg-gray-900/90 text-white text-[10px] px-1.5 py-0.5 rounded pointer-events-none opacity-80">{Math.round(el.width)} × {Math.round(el.height)}</div>}
                  {renderElement(el)}
                </Rnd>
              ))}
            </div>
        ...
        {/* 페이지 탭 */}
        <div className={`h-11 flex items-center px-4 ${T.panel} border-t ${T.border} z-20 gap-1.5 overflow-x-auto shrink-0`}>
          <Layers size={13} className="text-gray-500 mr-1.5 shrink-0"/>
          {pages.map((p,idx)=>(
            <button key={p.id} onClick={()=>{setSelectedIds([]);setCurrentPageIndex(idx);}} className={`px-3 py-1 rounded text-xs font-bold transition shrink-0 ${currentPageIndex===idx?'bg-emerald-500 text-white':'text-gray-500 hover:bg-gray-500/10'}`}>Page {idx+1}</button>
          ))}
          <button onClick={()=>{const p={id:`p_${Date.now()}`,elements:[]};setPages(prev=>[...prev,p]);setCurrentPageIndex(pages.length);}} className="px-2.5 py-1 rounded text-xs flex items-center gap-1 text-emerald-500 hover:bg-emerald-500/10 font-bold transition shrink-0"><Plus size={13}/>추가</button>
        </div>
      </main>
    </div>
  );
